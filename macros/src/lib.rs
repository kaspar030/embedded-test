...
fn tests_impl(args: TokenStream, input: TokenStream) -> parse::Result<TokenStream> {
    ...
    if macro_args.executor.is_some() {
        return Err(parse::Error::new(
            proc_macro2::Span::call_site(),
            "`#[embedded_test::tests]` attribute doesn't take an executor the feature `riot-rs` is enabled",
        ));
    }
    ...
    for test in &tests {
        ...
        let task_path = if cfg!(feature = "riot-rs") {
            quote!(riot_rs::task)
        } else {
        ...
        let entrypoint = if test.asyncness || init_is_async {
            ...
            if cfg!(feature = "riot-rs") {
                quote!(|| {
                    riot_rs::asynch::spawner().must_spawn(#ident_invoker());
    ...
    let (thread_start, maybe_export_name) = if cfg!(feature = "riot-rs") {
        (
            quote!(
